...
<div class="main-container">
    <div class="form-card">
        ...
        <p class="subtitle">
            @if(isset($adminMode))
                Редактирование портфолио пользователя <strong>{{ $portfolio->user->name ?? '' }}</strong>
            @else
                Заполните информацию о себе
            @endif
        </p>

        <form method="POST" action="{{ isset($adminMode) ? route('admin.portfolio.update', $portfolio->id) : route('portfolio.update') }}" id="portfolioForm" enctype="multipart/form-data">
        @csrf
            <div class="input-group">
                <label for="name">Название портфолио *</label>
                <input type="text" id="name" name="name" value="{{ $portfolio->title ?? '' }}" placeholder="Например: Веб-разработчик" required>
            </div>
            ...
            <div class="input-group">
                ...
                <input type="text" id="skills" name="skills" value="{{ isset($portfolio) ? $portfolio->skills->pluck ('name')->implode (',') : ''}}" placeholder="HTML, CSS, JavaScript, React, PHP">
            </div>

            <div class="section-title">🚀 Проекты</div>
                @foreach($portfolio->projects ?? collect() as $i => $project)
                <div class="input-group">
                <input name="projects[{{ $i }}][title]"
                    value="{{ $project->title }}"
                    placeholder="Название проекта">
                <input name="projects[{{ $i }}][description]"
                    value="{{ $project->description }}"
                    placeholder="Описание">
            </div>
            @endforeach
            @if(($portfolio->projects ?? collect())->isEmpty())
            ...
            <div id="imageInputs">
                ...
                @if(($portfolio->images ?? collect())->isNotEmpty())
                    <div class="images-grid" style="margin-bottom: 15px;">
                @foreach($portfolio->images as $image)
                <div class="image-card" data-id="{{ $image->id }}">
                    <img src="{{ asset('storage/' . $image->image_url) }}" alt="work">
                    <button type="button"
...
<script src="{{ asset('scripts/PortfolioEdit.js') }}"></script>
</html>
